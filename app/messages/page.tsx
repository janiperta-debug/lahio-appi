      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-800 font-sans mb-2">Viestit</h1>
              <p className="text-slate-600 leading-relaxed">
                Keskustele naapureiden kanssa turvallisesti ja yksityisesti
              </p>
            </div>
            {unreadCount > 0 && <Badge className="bg-cyan-800 text-white">{unreadCount} lukematonta</Badge>}
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 border-slate-300 focus:border-cyan-800 focus:ring-cyan-800 bg-white"
            />
          </div>
        </div>
            ...
            <Link key={conversation.id} href={`/messages/${conversation.id}`}>
              <Card
                className={`bg-white border-slate-200 hover:shadow-md transition-shadow cursor-pointer ${
                  conversation.unread ? "border-l-4 border-l-cyan-800" : ""
                }`}
              >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3
                          className={`font-semibold font-sans ${
                            conversation.unread ? "text-slate-900" : "text-slate-800"
                          }`}
                        >
                          {conversation.name}
                        </h3>
                      ...
                      <p
                        className={`text-sm truncate ${
                          conversation.unread ? "text-slate-700 font-medium" : "text-slate-600"
                        }`}
                      >
        ...
        {filteredConversations.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-slate-600 mb-2">Ei keskusteluja</h3>
            <p className="text-slate-500 mb-4">
              {searchQuery ? "Hakuehdoilla ei löytynyt keskusteluja." : "Aloita keskustelu naapureiden kanssa."}
            </p>
            <Link href="/discover">
              <Button className="bg-cyan-800 hover:bg-cyan-900 text-white">Löydä naapureita</Button>
            </Link>
          </div>
        )}
        ...
        <Card className="mt-8 bg-cyan-50 border-cyan-200">
          <CardContent className="pt-6">
            <div className="text-center">
              <MessageCircle className="w-8 h-8 text-cyan-800 mx-auto mb-3" />
              <h4 className="font-semibold text-cyan-800 text-sm mb-2">Turvallinen viestintä</h4>
